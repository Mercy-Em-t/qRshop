...
export default function SalesBrainManager() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white pb-20">
      <header className="border-b border-white/10 px-6 py-6 sticky top-0 z-10 bg-slate-900/80 backdrop-blur-md">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
           <div>
             <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
               <span className="text-indigo-400">🧠</span> Shop Brain Manager
             </h1>
             <p className="text-[10px] text-slate-400 uppercase tracking-[0.2em] mt-1 font-bold">AI Calibration Suite</p>
           </div>
           <button onClick={() => navigate('/a')} className="text-[10px] font-black text-slate-500 hover:text-white transition uppercase tracking-widest border border-white/10 px-4 py-2 rounded-full">Exit Suite</button>
        </div>
      </header>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <section className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-sm">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">AI Identity Name</label>
                <input 
                  type="text" 
                  ...
                  className="w-full bg-slate-800 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 transition font-bold text-white placeholder:text-slate-600"
                />
                <p className="text-[10px] text-slate-500">How should the customer address this agent?</p>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Engagement Tone</label>
                <select 
                  value={brain.tone}
                  onChange={e => setBrain({...brain, tone: e.target.value})}
                  className="w-full bg-slate-800 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-indigo-500 transition font-bold text-white"
                >
                ...
                <p className="text-[10px] text-slate-500">Influences the agent's vocabulary and pacing.</p>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-sm">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Operational Instructions</label>
                <textarea 
                  ...
                  className="w-full bg-slate-800 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-indigo-500 h-32 transition font-medium text-white placeholder:text-slate-600 resize-none"
                />
                <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Strategy mode: active</p>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Custom Shop Context</label>
                <textarea 
                  ...
                  className="w-full bg-slate-800 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-indigo-500 h-24 transition font-medium text-white placeholder:text-slate-600 resize-none"
                />
                <p className="text-[10px] text-slate-500">Specific facts the AI should know about your shop.</p>
              </div>
            </div>
          </section>
      ...
      {message && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-indigo-600 text-white px-8 py-4 rounded-2xl shadow-2xl animate-in font-bold text-xs uppercase tracking-widest z-50 border border-indigo-400 flex items-center gap-3">
