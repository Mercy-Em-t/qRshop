export default function Logo({ className = "h-8 w-8", textClassName = "font-black text-xl italic tracking-tighter" }) {
  return (
    <div className="flex items-center gap-2 group cursor-pointer">
      <div className={`${className} bg-safari-green rounded-xl flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform overflow-hidden relative`}>
        {/* Fusion: Acacia + Horizon Sun + Digital Lines */}
        <svg viewBox="0 0 100 100" className="w-4/5 h-4/5 text-white fill-current">
           {/* Horizon Sun (V1 Element) */}
           <path d="M10,85 Q50,40 90,85 Z" className="opacity-40" />
           {/* Acacia Trunk & Canopy (V2 Element) */}
      ...
      <span className={`${textClassName} text-gray-900`}>
        Modern <span className="text-safari-green">Savannah</span>
      </span>
    </div>
  );
}
