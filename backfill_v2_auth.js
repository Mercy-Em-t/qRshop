...
async function run() {
  ...
  const { data: { users }, error: listErr } = await admin.auth.admin.listUsers({ perPage: 500 });
  ...
  for (const su of shopUsers) {
    ...
    if (memberErr) {
      console.warn(`⚠️  Failed to migrate membership for user ${su.id} in shop ${su.shop_id}:`, memberErr.message);
    } else {
      process.stdout.write(".");
    }
  }
